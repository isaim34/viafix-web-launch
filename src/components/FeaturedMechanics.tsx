            ...
            <div key={mechanic.id} className="relative">
              {mechanic.featured && (
                <div className="absolute -top-2 -right-2 z-10">
                  <div className="bg-gradient-to-r from-yellow-400 to-yellow-500 text-black px-3 py-1 rounded-full text-xs font-bold shadow-lg flex items-center gap-1">
                    <Crown className="w-3 h-3" />
                    FEATURED
                  </div>
                </div>
              )}
              <div className={mechanic.featured ? 'ring-2 ring-yellow-400 rounded-lg' : ''}>
                <MechanicCard 
                  id={mechanic.id}
